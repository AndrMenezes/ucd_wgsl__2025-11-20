...
- Heterogeneity: complex cross-sample variation, $\operatorname{DI}(Y_j)=\operatorname{Var}(Y_j)/\mathbb{E}(Y_j)$ ranging from $41.15$ (_Ostrya_) and $470.18$ (_Cedrus_).
...
> - **Microbiome studies**: explicit account for zero-inflation in count-composition data,
but rely simple linear regression structure:
`r Citep(bib, c("Tang2018", "Zeng2023", "Koslovsky2023"))`.

--

.font120[Our contributions]
...
> We show that the zero-and-_N_-inflated Dirichlet-multinomial (ZANIDM) distribution
...
is performed by MCMC methods using Bayesian backfitting `r Citep(bib, "Hastie2000")`.
...
- *population-level* count probabilities, $\pmb{\theta}$ `r Citep(bib, "Murray2021")`:
...
- *population-level*  structural zero probabilities, $\pmb{\zeta}$ `r Citep(bib, "Chipman2010")`:
...
and ZANIDM linear regression model of `r Citet(bib, "Koslovsky2021")`.
...
Northern Hemisphere, along with three climate covariates `r Citep(bib, "Parnell2015")`: 
  - growing degree days above five $^\circ$ (GDD5): growing season warmth.
...
unknown covariates effects.
  ...
  - Develop MCMC algorithm for ZANIM-BART and ZANIM-LN-BART models combining
  ZANIM data augmentation `r Citep(bib, "Menezes2025")` established BART sampling routines.
  
  - Illustrate the useful of our models in the analysis of modern pollen-climate data.
...
**Future work**
  ...
  - Relax independence across $\mathbf{z}_i$ using the
